    ...
    function _validate_request($segments)
    {
        ...
        if (is_dir(APPPATH.'controllers/'.ucfirst($segments[0])))
        {
            ...
            do
            {
                ...
                $dir .= ucfirst($segments[0]);
                $segments = array_slice($segments, 1);
            } while (count($segments) > 0 && is_dir(APPPATH.'controllers/'.$dir.'/'.ucfirst($segments[0])));
           
            // Set the directory and remove it from the segment array
            $this->set_directory($dir);
            // @edit: END
            // @edit: If no controller found, use 'default_controller' as defined in 'config/routes.php'
            if (count($segments) > 0 && ! file_exists(APPPATH.'controllers/'.$this->fetch_directory().ucfirst($segments[0]).EXT))
            {
